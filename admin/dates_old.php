...
$next_order['coordinator_id']='asc';
		...
		<div class="content-header">
			<div class="btn-wrap">
				<h2><a href="#" class="back-btn"><span class="fa fa-arrow-circle-left"></span></a> '."Прошедшие свидания".' </h2>
...
$first_show_values = '[{"text":"парень", "value":"male"},{"text":"девушка", "value":"female"}]';
		...
		$html = '
			<h1 style="line-height: 30px"> Редактирование <br /><small>'."Прошедшие свидания".' #'.$id.'</small></h1>
			<form class="form" enctype="multipart/form-data" method="POST">
...
$coordinator_id_values = json_encode(q("SELECT name as text, id as value FROM coordinators", []));
	...
	function get_data($force_kill_pagination=false)
	{
		...
		$where = "dt<now() and status='scheduled'";
		if($where != "")
		{
			if($filter!='' || $srch !='')
			{
	...
	echo masterRender("Прошедшие свидания", $content, 10);
