...
$next_order['img']='asc';
		...
		<script>
				...
				{
					$(\'.big-icon\').html(\'<i class="fas fa-passport"></i>\');
				};
		...
		<div class="content-header">
			<div class="btn-wrap">
				<h2><a href="#" class="back-btn"><span class="fa fa-arrow-circle-left"></span></a> '."Документы".' </h2>
		...
		<div>'.
		"Документы ".GLOBAL_STORAGE::$parent_object["name"].""
		.'</div>';
...
<th></th>
			...
			<th>
				   <a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=img&sort_order='. ($next_order['img']) .'\' class=\'sort\' column=\'img\' sort_order=\''.$sort_order['img'].'\'>Документ'. $sort_icon['img'].'</a>
			</th>
					<th></th>
				</tr>
		</thead><tbody>';
...
<td>
	...
	$actions['edit'] = function()
	{
		...
		if(isset($id))
		{
			$item = q("SELECT * FROM documents WHERE id=?",[$id]);
			$item = $item[0];
		}
		            ...
		            <p>
		              '.$item["id"].'
						...
						<div class="form-group">
							<label class="control-label" for="textinput">Документ</label>
							<div class="">
	...
	$actions['edit_page'] = function()
	{
		...
		$html = '
			<h1 style="line-height: 30px"> Редактирование <br /><small>'."Документы".' #'.$id.'</small></h1>
			<form class="form" enctype="multipart/form-data" method="POST">
	...
	$actions['reorder'] = function()
	{
		...
		{
			qi("UPDATE `documents` SET `orderby` = ? WHERE id = ?", [$i, $line[$i]]);
		}
...
$user_id = $_REQUEST['user_id'];
		...
		qi("INSERT INTO documents (`img`, `user_id`) VALUES (?, ?)", [$img, $user_id]);
		$last_id = qInsertId();
	...
	$actions['edit_execute'] = function()
	{
		...
		{
			$set = implode(", ", $set);
			qi("UPDATE documents SET $set WHERE id=?", [$id]);
		}
	...
	$actions['delete'] = function()
	{
		...
		try
		{
			qi("DELETE FROM documents WHERE id=?", [$id]);
			echo "1";
		}
		catch (Exception $e)
		{
	...
	function get_data($force_kill_pagination=false)
	{
		...
		if($pagination == 1)
		{
			$items = q("SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT  main_table.*  FROM documents main_table) temp $srch $filter $where $order LIMIT :start, :limit",
				[
		...
		{
			$items = q("SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT main_table.*  FROM documents main_table) temp $srch $filter $where $order", []);
			$cnt = qRows();
			$pagination = "";
		}
	...
	echo masterRender("Документы", $content, 3);
